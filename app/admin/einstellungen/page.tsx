...
import GoalSoundStudio from "../../../components/admin/GoalSoundStudio";
export default async function Page({searchParams}:{searchParams:Promise<{sound?:string}>}){
 ...
 const current=data?.value as {url?:string;name?:string;start_seconds?:number;end_seconds?:number|null}|undefined; const q=await searchParams;
 return <div className="mx-auto max-w-6xl"><p className="club-eyebrow">Vereinsmanager</p><h1 className="club-heading mt-2">Einstellungen</h1>
  {q.sound&&<div className="mt-5 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-4 text-sm font-bold text-emerald-300">{q.sound==="updated"?"Tor-Sound gespeichert.":q.sound==="trimmed"?"Tor-Sound-Ausschnitt gespeichert.":"Standard-Tor-Sound wiederhergestellt."}</div>}
  <div className="club-card mt-8 overflow-hidden"><div className="border-b border-white/10 bg-gradient-to-r from-club-burgundy/60 to-transparent p-6"><div className="flex items-center gap-3"><Music2 className="text-club-light-red"/><div><p className="club-eyebrow">HUJA Match Experience</p><h2 className="mt-1 text-xl font-black uppercase text-white">Eigener Tor-Sound</h2></div></div><p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400">Lade euren eigenen HUJA-Torjubel hoch. Dieser Sound wird bei neuen Toren im öffentlichen LiveCenter verwendet, wenn der Fan den Live-Sound aktiviert hat.</p></div>
   <div className="p-6">{current?.url?<div className="mb-6 rounded-2xl border border-white/10 bg-black/25 p-4"><p className="text-xs font-black uppercase tracking-wider text-zinc-500">Aktuell</p><p className="mt-1 font-black text-white">{current.name||"Eigener HUJA Tor-Sound"}</p><audio controls preload="metadata" src={current.url} className="mt-3 w-full"/>
   <GoalSoundStudio url={current.url} start={Number(current.start_seconds??0)} end={current.end_seconds==null?null:Number(current.end_seconds)} />
   </div>:<div className="mb-6 rounded-2xl border border-white/10 bg-black/25 p-4 text-sm text-zinc-400">Aktuell wird der HUJA Standard-Tor-Sound verwendet.</div>}
