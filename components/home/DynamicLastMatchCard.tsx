...
export default function DynamicLastMatchCard({
  match,
}: DynamicLastMatchCardProps) {
  if (!match) {
    return (
      <section id="last-match" className="club-section py-10">
        <div className="club-container">
          <div className="club-card p-6 text-sm leading-6 text-zinc-400">
            Aktuell ist noch kein beendetes Spiel eingetragen.
          </div>
        </div>
      </section>
    );
              ...
              <div className="space-y-2">
                {match.scorers?.length ? (
                  match.scorers.map((scorer) => (
                    ...
                    >
                      <span className="h-2 w-2 rounded-full bg-club-light-red shadow-[0_0_14px_rgba(239,51,64,0.75)]" />
