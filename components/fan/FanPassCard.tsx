...
type Entry={rank:number;displayName:string;points:number;levelName:string;isMe:boolean};
type Data={displayName:string;points:number;level:number;levelName:string;nextLevelPoints:number;tips:number;exactTips:number;votes:number;reactionMatches:number;badges:Badge[];rank:number;totalFans:number;leaderboard:Entry[]};
 ...
 return <section className="club-section py-6"><div className="club-container"><div className="club-card overflow-hidden border-club-light-red/20">
  ...
  {open&&<div className="border-t border-white/10 p-5">
   ...
   <div className="mt-5 rounded-2xl border border-white/10 bg-black/20 p-4">
    <div className="flex items-center justify-between gap-3"><p className="flex items-center gap-2 text-xs font-black uppercase tracking-wider text-zinc-400"><Trophy size={15}/> Saison-Rangliste</p><p className="text-xs font-black text-club-light-red">Dein Platz: #{data.rank}</p></div>
    <div className="mt-3 space-y-2">{data.leaderboard.slice(0,5).map((x)=><div key={`${x.rank}-${x.displayName}`} className={`flex items-center gap-3 rounded-xl border px-3 py-2 ${x.isMe?"border-club-light-red/40 bg-club-burgundy/30":"border-white/5 bg-white/[0.03]"}`}><div className="w-8 text-center text-lg font-black">{x.rank===1?"🥇":x.rank===2?"🥈":x.rank===3?"🥉":`#${x.rank}`}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-white">{x.displayName}{x.isMe?" · Du":""}</p><p className="text-[10px] uppercase text-zinc-500">{x.levelName}</p></div><p className="text-sm font-black text-club-light-red">{x.points} P</p></div>)}</div>
    <p className="mt-3 text-[11px] text-zinc-600">{data.totalFans} Fanpass{data.totalFans===1?"":"-Fans"} in der Saisonwertung.</p>
   </div>
