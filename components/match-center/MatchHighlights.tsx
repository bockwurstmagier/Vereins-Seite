...
export default function MatchHighlights({events,status}:{events:MatchCenterEvent[];status:"scheduled"|"live"|"finished"}){
 ...
 return <section className="club-card mt-6 overflow-hidden">
  ...
  <div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-6">{clips.map((e,i)=>{
   const linkedGoal=[...events].filter(x=>x.event_type==="goal"&&x.minute<=e.minute&&e.minute-x.minute<=3).sort((a,b)=>b.minute-a.minute)[0]??null;
   return <article key={e.id} className={`overflow-hidden rounded-3xl border bg-black/30 ${e.is_highlight?"border-amber-400/35":"border-white/10"}`}>
   ...
   {linkedGoal&&<p className="px-4 pb-4 text-[10px] font-black uppercase tracking-wider text-club-light-red">⚽ Tor-Moment · {linkedGoal.minute}. Minute</p>}
  </article>})}</div>
 </section>
}
